...
if __name__ == '__main__':
    ...
    config = "moco_resnet_grid.yaml"

    base_cmd = f"python main_linear.py --config-path scripts/linear_probe/ --config-name {config}"
    ...
    for dataset in datasets:
        ...
        store_ckpt = True if dataset in ['imagenet_42', 'Places365_h5'] else False

        for i, row in meta.iterrows():
            name = (dataset + '_' + row['model_name']).replace("=", "\=")


            ckpt_name = f"{row['model_name']}_{row['id']}_ep={args.epoch}.ckpt"
